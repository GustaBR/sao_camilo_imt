...
class _CadastroPageState extends State<CadastroPage> {
  ...
  Future<void> _cadastrar() async {
    ...
    String perfilFormatado = _perfilSelecionado.toLowerCase();
    if (perfilFormatado == 'médico') perfilFormatado = 'medico';

    try {
      if (perfilFormatado == 'atleta') {
        ...
        double pesoParsed = double.tryParse(_pesoController.text.replaceAll(',', '.')) ?? 0.0;

        String sexoFormatado = 'nao_informado';
        if (_sexoSelecionado == 'Masculino') sexoFormatado = 'masculino';
        if (_sexoSelecionado == 'Feminino') sexoFormatado = 'feminino';
        ...
        if (codigo != null) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Cadastro realizado! Seu código: $codigo'), backgroundColor: Colors.green),
          );
          if (mounted) Navigator.pushReplacementNamed(context, '/login');
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(_db.ultimoErro ?? 'Erro ao cadastrar atleta.'), backgroundColor: Colors.red),
          );
        }

      } else {
        String registro = '';
        if (perfilFormatado == 'medico') {
          registro = _crmController.text.trim();
        } else if (perfilFormatado == 'nutricionista') {
        ...
        if (profissional != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(_db.ultimoErro ?? 'Erro ao cadastrar profissional.'), backgroundColor: Colors.red),
          );
        }
      }
    } catch (erro) {
      ...
      child: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            initialValue: _perfilSelecionado, // Corrigido para evitar aviso de descontinuação
            decoration: const InputDecoration(
              labelText: 'Eu sou um...',
              border: OutlineInputBorder(),
  ...
  idade--;
            ...
            TextFormField(
              ...
              validator: (value) {
                ...
                double? pesoParsed = double.tryParse(value.replaceAll(',', '.'));
                if (pesoParsed == null || pesoParsed <= 0) return 'Informe um peso válido maior que zero';
                return null;
              },
            ),
            const SizedBox(height: 16),
            DropdownButtonFormField<String>(
              initialValue: _sexoSelecionado, // Corrigido para evitar aviso de descontinuação
              decoration: const InputDecoration(
                labelText: 'Sexo',
                border: OutlineInputBorder(),
              ...
              items: <String>['Masculino', 'Feminino', 'Prefiro não informar']
                  .map((String value) => DropdownMenuItem<String>(value: value, child: Text(value)))
                  .toList(),
              onChanged: _isLoading ? null : (String? novoSexo) {
                if (novoSexo != null) {
                  setState(() {
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: IconButton(
              ...
              onPressed: _isLoading ? null : () => setState(() => _senhaOculta = !_senhaOculta),
            ),
          ),
          validator: (value) {
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: IconButton(
              ...
              onPressed: _isLoading ? null : () => setState(() => _confirmarSenhaOculta = !_confirmarSenhaOculta),
            ),
          ),
          validator: (value) {
